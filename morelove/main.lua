		...
		if button == 1 then
			if game.state["menu"] then
				for index in pairs(buttons.menu_state) do
					buttons.menu_state[index]:checkpress(x, y, player.radius)
				end
			elseif game.state["ended"] then
				for index in pairs(buttons.ended_state) do
					buttons.ended_state[index]:checkpress(x, y, player.radius)
				end
			end
		end
	end
...
function love.load()
	...
	buttons.menu_state.play_game = button("Play game", start_new_game, nil, 120, 40)
	buttons.menu_state.exit_game = button("Exit game", love.event.quit, nil, 120, 40)

	buttons.ended_state.replay_game = button("Replay game", start_new_game, nil, 100, 50)
	buttons.ended_state.menu = button("Menu", change_game_state, "menu", 100, 50)
...
function love.update(dt)
	...
	if game.state["running"] then
		for i = 1, #enemies do
			if not enemies[i]:check_touched(player.x, player.y, player.radius) then
...
function love.draw()
	...
	if game.state["running"] then
		love.graphics.printf(
			tostring(math.floor(game.points)),
			fonts.large.font,
			0,
			10,
			love.graphics.getWidth(),
